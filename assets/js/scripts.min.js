...
function doCompare() {
    ...
    var labels = ["STATUS","ASY","NAV","DESCRIPTION","MNFR1","MNFR-Part#1","MNFR2","MNFR-Part#2","MNFR3","MNFR-Part#3","MNFR4","MNFR-Part#4","REFERENCE","QTY","VERSION"];
    ...
    csvContent += "Job Number,"+firstRow[1].split("-")[1]+"\r\n";
    csvContent += 'Description,"'+firstRow[2]+'"\r\n';
    csvContent += "Revision,"+firstRow[14]+"\r\n\r\n\r\n";
    ...
    <th scope="row" >MNFR1</th>\n
    <th scope="row" >MNFR-Part#1</th>\n
    <th scope="row" >MNFR2</th>\n
    <th scope="row" >MNFR-Part#2</th>\n
    <th scope="row" >MNFR3</th>\n
    <th scope="row" >MNFR-Part#3</th>\n
    <th scope="row" >MNFR4</th>\n
    <th scope="row" >MNFR-Part#4</th>\n
    ...
    $.each(common, function(a, b) {
        ...
        if (unMatched.length > 0) {
            ...
            var r = '"'+"CHANGED";
            var cl = [];
            $.each(unMatched, function(x, y){
                if (y == 12){
                    // Change in QTY
                    cl.push( (oldRow[y].length == 0 ? "Added QTY" : (newRow[y].length == 0 ? "Removed QTY": (parseInt(newRow[y])>parseInt(oldRow[y])?"QTY ++"+(parseInt(newRow[y]) - parseInt(oldRow[y])).toString():"QTY --"+(parseInt(oldRow[y]) - parseInt(newRow[y])).toString()))) );  
                } else {
                    // Change in other fields
                    cl.push( (oldRow[y].length == 0 ? "Added " : (newRow[y].length == 0 ? "Removed ": "Change in "))+labels[y] );
                }
            });
            r += '(' + cl.join(", ") + ')"';
            $.each(newRow, function(k, v) {
                if (k==13){return true;}
                d += $.inArray(k, unMatched) === -1 ? "      <td>" + v + "</td>\n": "      <td class='cell-change'><span class='old'>"+(oldRow[k]||"--")+"</span><hr/><span>"+(v||"--")+"</span></td>\n";
                r += ',"'+v+'"';
            });
            csvContent += r+ "\r\n";
            d += "    </tr>\n";
    ...
    g = $("#table-preview");
    g.html(h);
    g.find("table").addClass("table table-responsive table-bordered");
    ...
    var d = new Date();
    var fileName = "Report-" + d.getTime() + ".csv";
